...
export async function fetchAllReviews(onBatchComplete, onComplete) {
  ...
  for (let i = 0; i < pageNumbers.length; i += BATCH_SIZE) {
    ...
    results.forEach((result) => {
      console.log(`[MosaicLens] Batch Item Status: ${result.status}`, result.status === 'fulfilled' ? `Length: ${result.value.length}` : `Error: ${result.reason}`);
    ...
    if (failedPages > 5) {
      console.warn(`[MosaicLens] Warning: ${failedPages} pages failed to load. Totals may be incomplete.`);
    }
