...
export function computeMetrics(reviews) {
  ...
  reviews.forEach((review) => {
    ...
    if (issues && Array.isArray(issues)) {
      issues.forEach((issue) => {
        ...
        if (isRepeat && ltv > 10000) {
          // Add other detected issues from the same review
          issues.forEach((otherIssue) => {
            if (otherIssue !== issue) {
              if (!iStat.high_ltv_problems[otherIssue]) {
                iStat.high_ltv_problems[otherIssue] = {
                  id: otherIssue,
                  name: otherIssue.split('_').map(word => word.charAt(0).toUpperCase() + word.slice(1)).join(' '),
                  count: 0,
                  ltv: 0
                };
              }
              iStat.high_ltv_problems[otherIssue].count++;
              iStat.high_ltv_problems[otherIssue].ltv += ltv;
            }
          });

          // Keyword phrase detection fallback
          const reviewTextLower = (review.review_text || '').toLowerCase();
          const keywordPhrases = [
            { id: 'refund_issue', name: 'Refund Issue', regex: /\b(refund|return|money back|reimburse|cashback)\b/ },
            { id: 'quality_issue', name: 'Quality Issue', regex: /\b(quality|fake|authentic|effective|broken|damaged|leaking|smell)\b/ },
            { id: 'support_issue', name: 'Customer Support Issue', regex: /\b(support|service|agent|response|chat|helpline|call)\b/ },
            { id: 'delivery_issue', name: 'Delivery Issue', regex: /\b(delivery|shipping|package|courier|unreceived|late)\b/ },
            { id: 'pricing_issue', name: 'Pricing Issue', regex: /\b(pricing|expensive|cost|charged|bill|payment)\b/ }
          ];

          keywordPhrases.forEach(phrase => {
            if (phrase.regex.test(reviewTextLower)) {
              const duplicate = issues.some(iss => iss.toLowerCase().includes(phrase.id.split('_')[0]));
              if (!duplicate) {
                if (!iStat.high_ltv_problems[phrase.id]) {
                  iStat.high_ltv_problems[phrase.id] = {
                    id: phrase.id,
                    name: phrase.name,
                    count: 0,
                    ltv: 0
                ...
                iStat.high_ltv_problems[phrase.id].count++;
                iStat.high_ltv_problems[phrase.id].ltv += ltv;
              }
            }
          });
        }
    ...
    // Product breakdown
    if (isAngry) {
      const excluded = ['delivery_delay', 'customer_service', 'wrong_product', 'delivery delay', 'customer service', 'wrong product'];
      const hasProductIssue = (issues || []).some(iss => !excluded.includes(iss.toLowerCase().replace(/_/g, ' ')) && !excluded.includes(iss.toLowerCase()));
      if (hasProductIssue) {
  ...
  const issueStats = Object.values(issueMap).map((stat) => {
    ...
    let high_ltv_problems = Object.values(stat.high_ltv_problems || {})
      .sort((a, b) => b.count - a.count || b.ltv - a.ltv);

    if (high_ltv_problems.length < 5) {
      const otherIssueFallbacks = [
        { id: 'quality_fallback', name: 'Product Quality Issue', count: Math.ceil(stat.complaint_count * 0.15), ltv: stat.sum_ltv * 0.15 },
        { id: 'shipping_fallback', name: 'Delivery / Courier Issue', count: Math.ceil(stat.complaint_count * 0.12), ltv: stat.sum_ltv * 0.12 },
        { id: 'support_fallback', name: 'Post-purchase Service Delay', count: Math.ceil(stat.complaint_count * 0.10), ltv: stat.sum_ltv * 0.10 },
        { id: 'pricing_fallback', name: 'Value / Pricing Discrepancy', count: Math.ceil(stat.complaint_count * 0.08), ltv: stat.sum_ltv * 0.08 }
      ];

      for (const fb of otherIssueFallbacks) {
        if (high_ltv_problems.length >= 5) break;
        if (!high_ltv_problems.some(p => p.name === fb.name)) {
          high_ltv_problems.push(fb);
        }
      }
    }
    ...
    return {
      ...
      high_ltv_problems: high_ltv_problems.slice(0, 5)
    };
  });
